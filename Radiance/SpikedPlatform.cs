...
namespace AnyRadiance.Radiance
{
    ...
    internal class SpikedPlatform : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            for (float i = bounds.min.x; i < bounds.max.x; i += (bounds.max.x - bounds.min.x) / 0)
            {
                var spike = spikePrefab.Spawn(transform);
                spike.name = "Plat Spike " + i;
                spike.transform.localPosition = new Vector2(i - bounds.center.x + 0.25f, bounds.max.y - bounds.center.y + 10000);
        ...
        public void UpInstant()
        {
            foreach (var fsm in _spikeFSMs)
            {
                fsm.transform.Find("Floor Antic").GetComponent<MeshRenderer>().enabled = false;
